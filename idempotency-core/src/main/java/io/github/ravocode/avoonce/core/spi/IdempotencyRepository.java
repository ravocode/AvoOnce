...
public interface IdempotencyRepository {
     ...
     * If a record is in a STARTED state and has not expired, this method should
     * throw an {@link IdempotencyConflictException}.
     ...
     * @return An optional containing the existing record if it's completed, or an
     *         empty optional if a new lock was acquired.
     * @throws IdempotencyConflictException if a conflicting, non-expired request is
     *                                      in progress.
     */
    Optional<IdempotencyRecord> acquireOrGet(String idempotencyKey);

    /**
     * Atomically attempts to save a new record with the state STARTED, verifying
     * the request hash.
     * If the record already exists but the request hashes do not match, an
     * {@link IdempotencyMismatchException} is thrown.
     *
     * @param idempotencyKey The unique key for the request.
     * @param requestHash    The hash of the request payload to ensure the key is
     *                       not reused for a different payload.
     * @return An optional containing the existing record if it's completed, or an
     *         empty optional if a new lock was acquired.
     * @throws IdempotencyConflictException if a conflicting, non-expired request is
     *                                      in progress.
     * @throws IdempotencyMismatchException if the key exists but the request hashes
     *                                      do not match.
     */
    ...
    /**
     * Updates an existing record with a successful payload and sets the state to
     * COMPLETED.
     * 
     * @param idempotencyKey The unique key for the request.
